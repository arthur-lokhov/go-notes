...
*   **Aeza:** Часто рекомендуется, поскольку исторически лоялен к клиентам из РФ и предлагает удобные способы оплаты.
...
![аренда_сервера](../assets/server.png)
...
4.  Выберите образ операционной системы: **Ubuntu 24.04**.
...
![сервер](../assets/admin_panel_for_server.png)
...
!!! warning "Ввод пароля вслепую"
    При вводе пароля в терминале **Linux** вы не увидите ни символов, ни звёздочек. Это сделано для безопасности. Просто аккуратно введите пароль и нажмите **Enter**.
...
### 🛡️ 4.3. Шаг 3: Укрепление безопасности (Hardening)
...
Создадим нового пользователя с обычными правами и дадим ему ваше имя (или любой ник, например `` `devops` ``).
```bash
adduser devops
```
Система попросит вас дважды ввести новый пароль для этого пользователя. **Придумайте сложный пароль.** Затем предложит ввести дополнительную информацию (имя, телефон) — можете просто нажимать **Enter**, оставляя поля пустыми.

**3. Предоставление прав администратора:**
Теперь нам нужно дать нашему новому пользователю возможность выполнять команды от имени `` `root` ``, когда это необходимо. Для этого мы добавим его в группу `` `sudo` ``.
```bash
...
*   `` `devops` ``: Имя нашего пользователя.

!!! success "Ключевой этап: Настройка доступа по SSH-ключу"

    Пароли можно подобрать. SSH-ключи — практически невозможно. Это самый важный шаг в защите вашего сервера. Давайте разберем его подробно.

    **Что такое SSH-ключи?**
    Это пара криптографических файлов:
    *   **Приватный ключ** (`id_ed25519`): Ваш секрет. Он хранится только на вашем компьютере и никогда его не покидает. Это как ваш паспорт или отпечаток пальца — он доказывает, что вы — это вы.
    *   **Публичный ключ** (`id_ed25519.pub`): Ваш замок. Вы можете свободно копировать его и размещать на любых серверах, к которым хотите иметь доступ. Он не является секретным.
    ...
    **1. Создание ключей на вашем локальном компьютере**
    Если у вас еще нет SSH-ключей, создайте их. Откройте терминал **на вашем компьютере** (не на сервере) и выполните команду. Мы будем использовать современный и безопасный алгоритм `ed25519`.

    ```bash
    ssh-keygen -t ed25519 -C "ваш_email@example.com"
    ```
    *   На все вопросы просто нажимайте **Enter**. Это создаст ключ со стандартным именем в стандартной папке (`~/.ssh/`) без дополнительной парольной фразы.
    ...
    **3. Копирование публичного ключа на сервер**
    Теперь нужно положить наш публичный "замок" на сервер. Для этого есть элегантная команда. Выполните её **на вашем локальном компьютере**:
    ```bash
    ssh-copy-id devops@ВАШ_IP_АДРЕС
    ```
    Она попросит пароль от пользователя `devops`, который мы создали ранее. После ввода пароля, ваш публичный ключ будет автоматически добавлен в нужный файл на сервере (`~/.ssh/authorized_keys`).

!!! warning "Отключаем вход по паролю и для root"
    Теперь, когда у нас есть пользователь `` `devops` `` и настроен вход по ключу, мы можем наконец-то запретить вход по паролю и под именем `` `root` ``. Это значительно повысит безопасность.

    *   Подключитесь к серверу под новым пользователем: `` `ssh devops@ВАШ_IP_АДРЕС` ``.
...
!!! danger "Критически важно!"
    Не закрывайте текущую сессию! Откройте **новый** терминал и попробуйте подключиться снова (`` `ssh devops@ВАШ_IP_АДРЕС` ``). Если всё прошло успешно (вас должно пустить без пароля), значит, вы всё сделали правильно. Теперь вход на ваш сервер возможен только с вашего компьютера по ключу.

**6. Настройка базового файрвола (`ufw`):**
Последний штрих в укреплении — настройка брандмауэра. Мы будем использовать `` `ufw` `` (**Uncomplicated Firewall**).
```bash
# Разрешаем подключения по SSH. Если вы сменили порт, укажите его.
...
sudo ufw status
```
Вы должны увидеть, что он активен и разрешает трафик на порты `22` (**SSH**) и `8443`.

**7. Настройка Часового Пояса:**
Это небольшой, но важный шаг для корректного отображения времени в логах и системных сообщениях. Установим, к примеру, центральноевропейское время.
```bash
sudo timedatectl set-timezone Europe/Berlin
```
Проверить можно командой `` `date` ``.

**8. Установка `Fail2Ban` — наш автоматический телохранитель:**
**Fail2Ban** — это сервис, который следит за логами и автоматически блокирует **IP-адреса**, с которых идут многократные неудачные попытки входа. Это лучшая защита от автоматизированных атак (**брутфорса**).
```bash
...
```
**Fail2Ban** по умолчанию уже содержит конфигурацию для защиты **SSH**. Чтобы посмотреть, кто был заблокирован, можно использовать команду `` `sudo fail2ban-client status sshd` ``.

**9. Настройка автоматических обновлений безопасности (`unattended-upgrades`):**
Чтобы не заходить на сервер каждую неделю для установки обновлений, мы можем поручить это системе. Пакет `` `unattended-upgrades` `` будет автоматически устанавливать самые важные, критические обновления безопасности.
```bash
# Устанавливаем пакет
...
!!! success "Крепость построена!"
    **Фух!** Этот этап был самым длинным и, возможно, сложным, но теперь ваш сервер — это настоящая маленькая крепость, которая к тому же будет сама о себе заботиться. Мы готовы к установке основного программного обеспечения.

---

## 🎯 Глава 5: Выбор сайта для маскировки — Искусство мимикрии

Это один из самых важных шагов, от которого напрямую зависит стабильность и незаметность вашего туннеля. Технология **Reality** работает, «притворяясь» другим, очень известным и популярным сайтом. Если выбрать неправильный сайт, системы **DPI** могут заметить аномалии и заподозрить неладное.

### 🤔 5.1. Критерии идеального сайта-прикрытия

Не любой сайт подходит на роль «донора». Вот ключевые критерии:

1.  **Популярность и репутация:** Сайт должен принадлежать крупной, известной международной компании (**Microsoft**, **Google**, **Apple**, **Amazon**, **Cloudflare** и т.д.). Трафик к таким сайтам огромен, и ваш маленький ручеёк в нём просто затеряется.
2.  **Современный TLS:** Сайт должен поддерживать современные версии протокола **TLS** (в идеале **TLS 1.3**) и использовать надёжные шифры. Это гарантирует, что «рукопожатие» будет выглядеть стандартно для систем анализа трафика.
3.  **Хорошая сетевая связность:** Серверы сайта должны быть расположены на крупных **CDN** (Content Delivery Network) и иметь хорошую сетевую доступность с вашего **VPS**. Это напрямую влияет на скорость и задержку вашего соединения.
...
!!! warning "Не используйте сайты из примеров!"
    Не стоит всем использовать `www.microsoft.com` или `www.google.com`. Если тысячи пользователей будут использовать одни и те же сайты для маскировки, это может стать сигналом для систем блокировок. Ищите свой, уникальный вариант.

### 🕵️ 5.2. Поиск идеального кандидата с помощью RealiTLScanner

Вручную проверять сотни сайтов — долго и неэффективно. К счастью, есть специальный инструмент — **RealiTLScanner**. Он автоматически проверяет список доменов, анализирует их **TLS**-сертификаты и измеряет задержку, выдавая итоговую оценку пригодности.

#### Установка и запуск сканера

Подключитесь к вашему **VPS** и выполните следующие команды:
...
2.  **Скачаем последнюю версию RealiTLScanner:**
    !!! info "Поиск актуальной версии"
        Перейдите на [страницу релизов RealiTLScanner](https://github.com/XTLS/RealiTLScanner/releases) и скопируйте ссылку на последний архив для архитектуры `linux_x64`.
    
    ```bash
    # Замените URL на актуальный, если необходимо
    curl -Lo RealiTLScanner.zip https://github.com/XTLS/RealiTLScanner/releases/download/v1.1.5/RealiTLScanner_linux_x64.zip
    ```

3.  **Распакуем архив и дадим права на исполнение:**
    ```bash
    unzip RealiTLScanner.zip
    chmod +x RealiTLScanner
    ```

4.  **Запускаем сканирование:**
    Теперь самое интересное. Запустим сканер. Он будет использовать встроенный список популярных доменов.
    ```bash
    ./RealiTLScanner
    ```
    Процесс может занять несколько минут. Сканер будет проверять каждый домен и выводить результаты в реальном времени.

### 📊 5.3. Анализ результатов

После завершения сканирования вы увидите таблицу с результатами. Нас интересуют следующие столбцы:

*   `Domain`: Имя домена.
*   `Hello Delay`: Задержка **TLS Handshake**. Чем меньше, тем лучше.
*   `Grade`: Итоговая оценка. `A+` и `A` — наши лучшие кандидаты.

!!! example "Пример вывода RealiTLScanner"
    ```
    Domain                 Hello Delay  Grade  HTTP Delay  Issues
    ---------------------  -----------  -----  ----------  ------
    www.apple.com          2ms          A+     2ms         (none)
    www.digitalocean.com   5ms          A+     5ms         (none)
    www.ubuntu.com         15ms         A      15ms        (none)
    ...
    ```

!!! success "Выбор победителя"
    Выберите из списка домен с оценкой **`A+`** или **`A`** и минимальной задержкой (`Hello Delay`). Это и будет ваш идеальный кандидат для поля `dest` и `serverNames` в настройках **Reality**. Запишите или скопируйте его. Например, в данном случае `www.apple.com` — превосходный выбор.

Теперь, вооружившись идеальным доменом для маскировки, мы можем переходить к настройке самого **Xray**.

---

## ⚙️ Глава 6: Установка и Настройка Xray — Сердце Нашего Туннеля

Двигаемся дальше. Сервер полностью готов и защищён. Теперь переходим к самому интересному — установке и настройке программного обеспечения, которое и будет создавать наш невидимый туннель.

!!! info "Зачем нужна панель управления?"
    Мы будем использовать удобную панель управления **3x-ui**, которая предоставляет графический интерфейс для управления **Xray**. Это избавляет нас от необходимости вручную редактировать сложные файлы конфигурации в формате **JSON**, что значительно снижает порог входа и вероятность ошибки.

### 📥 6.1. Шаг 1: Установка панели 3x-ui

Разработчики **3x-ui** предоставляют удобный скрипт для автоматической установки. Он сам определит вашу систему, скачает необходимые файлы и настроит базовую конфигурацию.

Подключитесь к вашему серверу под пользователем `` `devops` `` и выполните следующую команду:
...
!!! question "Что делает эта команда?"
    ...
    *   `` `bash <(...)` ``: Передаёт скачанный скрипт напрямую в командный интерпретатор `` `bash` `` для выполнения, не сохраняя его на диске.

В процессе установки скрипт задаст вам несколько вопросов:
*   `Continue? (y/n)`: Введите `` `y` ``.
*   `Please enter a username for the panel`: Придумайте имя пользователя для входа в панель.
*   `Please enter a password for the panel`: Придумайте сложный пароль.
*   `Please enter a port for the panel`: Введите порт, на котором будет доступна панель. Можете оставить стандартный `` `2053` `` или выбрать другой (например, `` `54321` ``). **Запомните этот порт!**

После завершения установки скрипт покажет вам информацию о том, как получить доступ к панели.

!!! note "Открываем порт в файрволе"
    Не забудьте разрешить доступ к порту панели в `` `ufw` ``. Если вы выбрали порт `` `2053` ``, команда будет такой:
    ```bash
    sudo ufw allow 2053/tcp
    sudo ufw reload
    ```

Теперь вы можете открыть панель в браузере, перейдя по адресу `` `http://ВАШ_IP_АДРЕС:2053` `` (или другой порт, который вы указали). Введите логин и пароль, которые вы задали при установке.

### 🔗 6.2. Шаг 2: Создание нашего невидимого подключения

Мы внутри панели. Её интерфейс достаточно прост. Нас интересует раздел **"Входящие"** (`` `Inbounds` ``).

1.  Перейдите в раздел **"Входящие"** в меню слева.
2.  Нажмите зелёную кнопку **"+ Добавить входящее"**.
3.  Перед вами откроется форма для настройки нового подключения. Заполним её.

!!! example "Параметры подключения VLESS + Reality"
    *   **Название** (`` `Remark` ``): Дайте подключению осмысленное имя, например `` `MyRealityLink` ``.
    *   **Протокол** (`` `Protocol` ``): Выберите **VLESS**.
    *   **Порт** (`` `Port` ``): Укажите `` `8443` ``. Это тот порт, который мы уже открыли в файрволе в предыдущей главе.
    *   **Пользователи** (`` `Users` ``):
        *   Нажмите синюю кнопку **"+ Добавить пользователя"**.
        *   **UUID**: Это ваш уникальный идентификатор, по сути — главный пароль. Он генерируется автоматически. Никому его не показывайте.
        *   **Flow**: Выберите `` `xtls-rprx-vision` ``. Это самый современный и производительный режим управления потоком для **VLESS**.
    *   **Безопасность** (`` `Security` ``):
        *   **Тип** (`` `Security` ``): Выберите `` `reality` ``. Это активирует наш «плащ-невидимку».
        *   **SNI** (`` `Server Names` ``): Вставьте сюда домен, который вы выбрали в **Главе 5**. Можно указать несколько через запятую, но для **Reality** важен только первый в списке, который должен совпадать с полем `Dest`.
        *   **Dest**: Укажите здесь тот же домен, что и в **SNI**, с портом `` `443` ``. Например: `` `www.apple.com:443` ``.
        *   **Ключи** (`` `Public Key` `` / `` `Private Key` ``): Нажмите кнопку **"Сгенерировать ключи"**. Панель сама создаст необходимую криптографическую пару.
        *   **Short IDs**: Нажмите кнопку **"Сгенерировать Short ID"** несколько раз, чтобы создать 2-3 коротких идентификатора.

Проверьте все настройки ещё раз и нажмите зелёную кнопку **"Добавить"**.

!!! info
    Ваше первое подключение создано! Панель автоматически перезапустит **Xray** с новой конфигурацией.

### 📲 6.3. Шаг 3: Получение ссылки для клиента

Теперь нам нужно передать все эти сложные настройки нашему клиентскому приложению (на телефоне или компьютере). Панель **3x-ui** делает это максимально просто.
...
!!! success "Сервер настроен!"
    **Поздравляю!** Серверная часть полностью настроена. Мы создали защищённый туннель, который мастерски маскируется под обычный веб-трафик к крупному сайту. В **следующей главе** мы разберёмся, какие клиентские приложения использовать и как их настроить.

---

## 🔐 Глава 7 (Опционально): Настройка Pi-hole — Ваш сетевой щит

Вы настроили прокси — это уже огромный шаг к свободе и безопасности. Теперь давайте сделаем его еще лучше, добавив собственный DNS-сервер с функцией блокировки рекламы и трекеров. Это превратит ваш VPS в полноценный бастион приватности.
...
### 🐳 7.1. Концепция установки через Docker
...
1.  **Установка Docker и Docker Compose:** Сначала на ваш сервер нужно установить Docker и Docker Compose. Это стандартная процедура, для которой есть официальные инструкции.
    ```bash
...
2.  **Создание `docker-compose.yml`:** Вы создаете файл `docker-compose.yml`, в котором описываете сервис для Pi-hole. Ключевые моменты в этом файле:
    ...
    *   `volumes`: Создаем постоянные хранилища для конфигурации Pi-hole (`/etc/pihole`) и его DNS-кэша (`/etc/dnsmasq.d`). Это гарантирует, что ваши настройки и списки не пропадут после перезапуска контейнера.
...
### 🔗 7.2. Интеграция Pi-hole с вашим VPN-клиентом
...
!!! success "Как это работает?"
    Когда VPN активен, клиентское приложение перехватывает все DNS-запросы. Увидев в настройках IP-адрес вашего сервера, оно отправляет эти запросы не куда-то в интернет, а **внутрь зашифрованного туннеля** на ваш сервер. На сервере запрос попадает на порт 53, где его уже слушает Pi-hole. Pi-hole обрабатывает запрос (блокируя рекламу) и отдает ответ, который так же по туннелю возвращается вам.
