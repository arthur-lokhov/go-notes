...
!!! note "Модуль — это минимальная единица версионирования в Go, объединяющая один или несколько пакетов."
...
!!! info "До Go 1.11 код размещался в директории `$GOPATH/src`, и сборка была возможна только оттуда."
...
!!! note "Файл `go.mod` описывает имя модуля, зависимости, версии языка и настройки сборки."
...
!!! warning "Начиная с Go `1.17` версия влияет на стратегию разрешения зависимостей и возможности компилятора (например, generics — c `1.18`)."
...
!!! danger "Если используешь `replace`, редактор должен запускаться из корня, где они лежат, иначе IDE будет «слепой» к локальным путям, и поведение будет отличаться от `go build`."
...
!!! warning "`replace` не распространяется — влияет только на локальную сборку."
...
!!! warning "Работает только если модуль запускается как `main` или `test`. Не работает, если модуль - зависимость."
...
!!! warning "Даже собственные подпакеты могут не импортироваться без `go.mod`."
...
!!! warning "Не редактируйте `go.sum` вручную — это вызовет ошибки валидации."
...
!!! note "Go не требует центрального репозитория пакетов (как npm или PyPI). Источником истины является ваш Git-репозиторий и его теги."
...
!!! note "Мажорная версия `v2+` должна быть в пути модуля: `github.com/myorg/mylib/v2`."

---

## 🛠️ Управление модулями

!!! note "`go mod` — это набор команд для управления модулями и зависимостями."

### `go mod init`

Инициализирует новый модуль в текущем каталоге, создавая файл `go.mod`. Если путь к модулю не указан, команда попытается угадать его.
...
Загружает модули, указанные в `go.mod`, в локальный кэш. Полезно для предварительного заполнения кэша в CI/CD.
...
Создает каталог `vendor/` с копиями всех зависимостей проекта. Это позволяет создавать полностью изолированные сборки.
